...
export default function NotificationCenter({ onClose }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        // ✅ getPushSubscriptions now returns an array directly
  ...
  return (
    <div className="absolute right-0 mt-2 w-[380px] bg-white rounded-xl shadow-lg border border-slate-100 z-30 overflow-hidden">
      ...
      <div className="max-h-72 overflow-auto">
        {loading ? (
          <div className="p-4 text-sm text-sprada2/70">Loading...</div>
        ) : items.length === 0 ? (
          ...
          items.map((s) => (
            <div key={s.id} className="p-3 border-b last:border-b-0 flex items-start gap-3">
              <div className="w-10 h-10 rounded-md bg-sprada1/30 flex items-center justify-center text-sprada4">
                🔔
              </div>
      ...
      <div className="p-3 border-t text-sm text-center text-sprada2/60">
        Manage push subscriptions in admin
      </div>
    </div>
  );
}