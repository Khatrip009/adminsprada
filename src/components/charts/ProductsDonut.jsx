// src/components/ProductsDonut.jsx
...
export default function ProductsDonut({ top = 6 }) {
  ...
  async function load() {
    ...
    try {
      ...
      console.error("ProductsDonut load", err);
  ...
  if (!data.length) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-slate-800">
        <div className="text-sm text-sprada3 dark:text-slate-300">Donut</div>
        <div className="text-lg font-bold text-sprada2 dark:text-white">No data</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-slate-800">
      <div className="mb-2">
        ...
        <div className="text-lg font-bold text-sprada2 dark:text-white">Products donut</div>
      </div>

      <div style={{ width: "100%", height: 260, position: "relative" }}>
        <ResponsiveContainer>
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="name" innerRadius={60} outerRadius={90} paddingAngle={3} labelLine={false} label={({ percent }) => `${Math.round(percent * 100)}%`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-sprada2 dark:text-white">{total}</div>
          </div>
        </div>
      </div>
    </div>
